...
try{
  let orderCheck=await checkOrder(order);
  let paymentCheck=await payment(orderCheck);
  let result =await stockControl(paymentCheck)
  console.log(result)
}catch(err){
  console.log(err)
}
...
document.getElementById('submit_btn').addEventListener('click',makeOrder)